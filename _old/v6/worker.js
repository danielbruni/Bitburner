...
export async function main(ns) {
  ...
  try {
    ...
    if (action === 'hack') {
      ns.print(`💰 Hacked ${target} for $${formatMoney(result)} in ${executionTime.toFixed(2)}s`);
    } else if (action === 'grow') {
      ns.print(`📈 Grew ${target} by ${result.toFixed(2)}x in ${executionTime.toFixed(2)}s`);
    } else if (action === 'weaken') {
      ns.print(`🔒 Weakened ${target} by ${result.toFixed(4)} in ${executionTime.toFixed(2)}s`);
    }
    
    // For special RAM sizes like 16GB servers, execute different optimizations
    if (ns.getServerMaxRam(ns.getHostname()) === 16 && ns.getServerUsedRam(ns.getHostname()) > 0) {
      ...
      if (action === 'weaken') {
        ...
        if (money < maxMoney * 0.75) {
          ns.spawn('/v6/worker.js', 1, target, 'grow', threads, JSON.stringify(taskInfo));
        }
      } else if (action === 'grow') {
        // After grow, apply a weaken to counter security increase
        ns.spawn('/v6/worker.js', 1, target, 'weaken', threads, JSON.stringify(taskInfo));
      }
